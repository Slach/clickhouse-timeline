...
SELECT
    ...
    intDiv(sum(read_bytes),if(intervals=0,1,intervals)) as m
...
clickhouse-client -q "${METRIC_QUERY}" | rare heat -m "([^\t]+)\t([^\t]+)\t([^\t]+)" -e '{timeformat {time {1}} "15:04" }' -e "{2}" -e "{3}" --rows 50 --scale log2
